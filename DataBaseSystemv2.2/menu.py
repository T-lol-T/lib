...
class menu(object):
    ...
    def createWindow(self):
        ...
        # 界面标题
        L1 = Label(self.window,text = '校园选课管理系统', font=ft1, relief = "flat",bg="#87CEFA")
...
#---------------------------------------------------------------------------------------------------------
        ...
        Button(self.window, text='教师修改', relief='groove', command=self.teacher_operate).place(
            relx=0.74, rely=0.53, relwidth=0.2,anchor=CENTER)
        Button(self.window, text='课程修改', relief='groove', command=self.course_operate).place(
            relx=0.74, rely=0.61, relwidth=0.2,anchor=CENTER)
        Button(self.window, text='院系修改', relief='groove', command=self.dept_operate).place(
            relx=0.74, rely=0.69, relwidth=0.2,anchor=CENTER)
        # Button(self.window, text='学生信息', relief='groove',command=self.student_info).place(
        #     relx=0.74, rely=0.85, relwidth=0.2,anchor=CENTER)
        Button(self.window, text='考试管理', relief='groove', command=self.exam).place(
            relx=0.74, rely=0.77, relwidth=0.2, anchor=CENTER)
        #数据报表
        Button(self.window, text='数据报表', relief='groove',command=self.excel_data).place(
            relx=0.74, rely=0.85, relwidth=0.2,anchor=CENTER)
    ...
    def excel_data(self):
        if not self.handle:  # 只有handle为False时（代表无该窗口存在）才能执行建立窗口的语句
            ...
            Button(top, text='学生信息报表', relief='groove', command=self.student_excel).place(
                relx=0.5, rely=0.1, relwidth=0.3, anchor=CENTER)
            Button(top, text='教师信息报表', relief='groove', command=self.teahcer_excel).place(
                relx=0.5, rely=0.3, relwidth=0.3, anchor=CENTER)
            Button(top, text='课程信息报表', relief='groove', command=self.course_excel).place(
                relx=0.5, rely=0.5, relwidth=0.3, anchor=CENTER)
            Button(top, text='院系信息报表', relief='groove', command=self.dept_excel).place(
                relx=0.5, rely=0.7, relwidth=0.3, anchor=CENTER)
            Button(top, text='选课信息报表', relief='groove', command=self.selectcourse_excel).place(
    ...
    def student_excel(self):
        ...
        path = os.getcwd()#获取当前路径的信息
        messagebox.showinfo('提示！', f'学生信息报表创建成功!生成路径为{path}')#

    #教师信息报表
    def teahcer_excel(self):
        if self.user_handle:  # 如果是学生则退出
            showinfo(title='提示', message='您没有操作权限')
            return
        table_name = 'teacher_info'#
        excel_name = '教师信息报表.xlsx'#
        sheet= []
        con = pymysql.connect(host='localhost', user='root', password='root',
                          database='student', port=3306, charset='utf8')
        cur = con.cursor()
        ...
        path = os.getcwd()#获取当前路径的信息
        messagebox.showinfo('提示！', f'教师信息报表创建成功!生成路径为{path}')  #

    #课程信息报表
    def course_excel(self):
        if self.user_handle:  # 如果是学生则退出
            showinfo(title='提示', message='您没有操作权限')
            return
        table_name = 'course'#
        excel_name = '课程信息报表.xlsx'#
        sheet= []
        con = pymysql.connect(host='localhost', user='root', password='root',
                          database='student', port=3306, charset='utf8')
        cur = con.cursor()
        ...
        path = os.getcwd()#获取当前路径的信息
        messagebox.showinfo('提示！', f'课程信息报表创建成功!生成路径为{path}')  #
    ...
    def dept_excel(self):
        ...
        path = os.getcwd()#获取当前路径的信息
        messagebox.showinfo('提示！', f'院系信息报表创建成功!生成路径为{path}')  #

    #选课信息报表
    def selectcourse_excel(self):
        if self.user_handle:  # 如果是学生则退出
            showinfo(title='提示', message='您没有操作权限')
            return
        table_name = 'student_course'#
        excel_name = '选课信息报表.xlsx'#
        sheet= []
        con = pymysql.connect(host='localhost', user='root', password='root',
                          database='student', port=3306, charset='utf8')
        cur = con.cursor()
        ...
        path = os.getcwd()#获取当前路径的信息
        messagebox.showinfo('提示！', f'选课信息报表创建成功!生成路径为{path}')  #
    ...
    def course_selection(self):
        if not self.handle:  # 只有handle为False时（代表无该窗口存在）才能执行建立窗口的语句
            ...
            tree1 = ttk.Treeview(top, show='headings', column=(
                'cid', 'name', 'teacher', 'time', 'classroom', 'credit'))
            # 创建滚动条
            yscrollbar = tk.Scrollbar(tree, command=tree.yview)
            yscrollbar.pack(side=tk.RIGHT, fill=tk.Y)
            tree.config(yscrollcommand=yscrollbar.set)

#——————————————————————————————————————————————————————————————————————————————————————————————
            yscrollbar1 = tk.Scrollbar(tree1, command=tree1.yview)
            yscrollbar1.pack(side=tk.RIGHT, fill=tk.Y)
            tree1.config(yscrollcommand=yscrollbar1.set)

            tree1.place(rely=0.37,relx=0.5, width=800, relheight=0.6)
            tree1.column('cid', width=100, anchor="center")
            tree1.column('name', width=200, anchor="center")
            tree1.column('teacher', width=100, anchor="center")
            tree1.column('time', width=150, anchor="center")
            tree1.column('classroom', width=120, anchor="center")
            tree1.column('credit', width=80, anchor='center')

            tree1.heading('cid', text='课程号')
            tree1.heading('name', text='课程名')
            tree1.heading('teacher', text='老师')
            tree1.heading('time', text='时间')
            tree1.heading('classroom', text='教室')
            tree1.heading('credit', text='学分')

            Button(top,text='显示所有课程信息',command=lambda: self.show_course_info(tree1)).grid(row=1,column=3,padx=200, ipadx=35)
#——————————————————————————————————————————————————————————————————————————————————————————————

            tree.place(rely=0.37, width=800, relheight=0.6)
        ...
        if ans is True:
            if self.sid.get() == "" or self.cid.get() == "":
                ...
                if result > 0:
                    ...
                    sql_2 = "insert into student_course values(%s,%s,%s,%s,NULL)"
                    result = cur.execute(
    ...
    def course_info(self):
        if not self.handle:  # 只有handle为False时（代表无该窗口存在）才能执行建立窗口的语句
            ...
            Button(top, text='显示所有课程信息', command=lambda: self.show_course_info(
    ...
    def teacher_info(self):
        if not self.handle:  # 只有handle为False时（代表无该窗口存在）才能执行建立窗口的语句
            ...
            Entry(top, textvariable=self.dept_name).grid(
                row=5, column=1, pady=5, ipadx=60)
            Button(top, text='显示所有教师信息', command=lambda: self.show_teacher_info(
    ...
    def student_operate(self):
        if not self.handle:  # 只有handle为False时（代表无该窗口存在）才能执行建立窗口的语句
            ...
            Button(top, text='修改学生信息', command=lambda: self.modify_stu_info(
                tree)).grid(row=3, column=2, padx=50, ipadx=50)
            Button(top, text='查询学生信息', command=lambda: self.search_stu_info(
                tree)).grid(row=4, column=2, padx=50, ipadx=50)
    ...
    def course_operate(self):
        if not self.handle:  # 只有handle为False时（代表无该窗口存在）才能执行建立窗口的语句
            ...
            Label(top, text='课程号:').grid(row=0, column=0, pady=5, ipadx=80, sticky=E)
            Label(top, text='课程名:').grid(row=1, column=0, pady=5, ipadx=80, sticky=E)
            Label(top, text='老师:').grid(row=2, column=0, pady=5, ipadx=80, sticky=E)
            Label(top, text='时间:').grid(row=3, column=0, pady=5, ipadx=80, sticky=E)
            Label(top, text='教室:').grid(row=4, column=0, pady=5, ipadx=80, sticky=E)
            Label(top, text='学分:').grid(row=5, column=0, pady=5, ipadx=80, sticky=E)
            Entry(top, textvariable=self.cid).grid(
                row=0, column=1, pady=5, ipadx=60)
            Entry(top, textvariable=self.course_name).grid(
                row=1, column=1, pady=5, ipadx=60)
    ...
    def dept_operate(self):
        if not self.handle:  # 只有handle为False时（代表无该窗口存在）才能执行建立窗口的语句
            ...
            top.protocol("WM_DELETE_WINDOW", lambda: self.quit(top))
            # ****************
            Label(top, text='学院名称:').grid(row=0, column=0, pady=5, ipadx=80, sticky=E)
            Label(top, text='专业名称:').grid(row=1, column=0, pady=5, ipadx=80, sticky=E)
            ...
            Button(top, text='所有院系信息', command=lambda: self.show_dept_info(
                tree)).grid(row=0, column=2, padx=50, ipadx=50)
            ...
            Button(top, text='修改院系信息', command=lambda: self.modify_dept_info(
                tree, top)).grid(row=2, column=2, padx=50, ipadx=50)
    ...
    def exam(self):  # 考试管理（登记分数）
        if not self.handle:  # 只有handle为False时（代表无该窗口存在）才能执行建立窗口的语句
            ...
            top.protocol("WM_DELETE_WINDOW", lambda: self.quit(top))
            tree = ttk.Treeview(top, show='headings', column=(
                'sid', 'stu_name', 'cid', 'course_name', 'score'))
            tree.place(rely=0.37, width=CurWid, relheight=0.6)
            tree.column('sid', width=150, anchor="center")
            tree.column('stu_name', width=200, anchor="center")
            ...
            Entry(top, textvariable=self.course_name).grid(
                row=3, column=1, pady=5, ipadx=60)
            Entry(top, textvariable=self.score).grid(
                row=4, column=1, pady=5, ipadx=60)
    ...
    def show_exam(self, tree):
        ...
        cur = con.cursor()
        if self.user_handle:  # 如果是学生则退出
            cur.execute("select * from student_course where sid = %s",
                        self.user_name)
        else:
            cur.execute("select * from student_course order by sid")
        lst = cur.fetchall()
    ...
    def update_score(self, tree):
        ...
        if ans is True:
            ...
            result = cur.execute(
                sql, (self.score.get(), self.sid.get(), self.cid.get()))
            if result == 0:
                showerror(title='提示', message='更新失败!')
            else:
                if float(self.score.get()) >= 60:
                    cur.execute(
                        "delete from exam_makeup where sid = %s and cid = %s", (self.sid.get(), self.cid.get()))
                con.commit()
    ...
    def search_exam(self, tree):
        ...
        if self.cid.get() != "" and self.sid.get() != "":
            sql = "select * from student_course where sid = %s and cid = %s"
            ...
            if self.sid.get() != "":
                result = cur.execute(
                    "select * from student_course where sid = %s", self.sid.get())
                if result > 0:
                    lst = cur.fetchall()
                    for item in lst:
            ...
            if self.name.get() != "":
                ...
                if temp == 0:  # 代表当前表格中没有数据
                    result = cur.execute(
                        "select * from student_course where stu_name like '%%%%%s%%%%'" % (self.name.get()))
                    if result > 0:
                        lst = cur.fetchall()
                        for item in lst:
                            tree.insert("", END, value=item)
            ...
            if self.cid.get() != "":
                ...
                if temp == 0:  # 代表当前表格中没有数据
                    result = cur.execute(
                        "select * from student_course where cid = %s", self.cid.get())
                    if result > 0:
                        lst = cur.fetchall()
                        for item in lst:
                            tree.insert("", END, value=item)
            ...
            if self.course_name.get() != "":
                ...
                if temp == 0:  # 代表当前表格中没有数据
                    result = cur.execute(
                        "select * from student_course where course_name like '%%%%%s%%%%'" % (self.course_name.get()))
                    if result > 0:
                        lst = cur.fetchall()
                        for item in lst:
                            tree.insert("", END, value=item)
            ...
            if self.score.get() != "":
                ...
                if temp == 0:  # 代表当前表格中没有数据
                    ...
                    result = cur.execute(
                        "select * from student_course where score = %s", self.score.get())
                    if result > 0:
                        lst = cur.fetchall()
                        for item in lst:
                            tree.insert("", END, value=item)
